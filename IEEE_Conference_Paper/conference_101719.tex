...
\begin{abstract}
Accurate assessment of grapevine leaf health and ripening stages is essential for precision viticulture, yet traditional manual inspection is labor-intensive, subjective, and often inconsistent. Recent advances in deep learning have enabled automated analysis, but most high-performing models are computationally intensive and require large datasets, limiting their applicability in resource-constrained vineyard settings. In this paper, we propose \textit{GrapevineNet\_SE}, a lightweight transfer learning model optimized for multi-stage grapevine leaf classification on a relatively small dataset (4,004 images). The model employs a frozen EfficientNet-B0 backbone, enhanced with a squeeze-and-excitation attention module, and a compact classification head, enabling efficient feature extraction and robust stage-wise discrimination. Extensive experiments demonstrate that \textit{GrapevineNet\_SE} achieves a validation accuracy of 92.25\% and a test accuracy of 95.26\%, significantly outperforming baseline EfficientNet-B0 (73.07\%) and ResNet50 (65.34\%) architectures while maintaining a low computational footprint suitable for real-time edge deployment. These results highlight the model's potential to facilitate timely disease monitoring, support vineyard management, and bridge the gap between high-performing research models and practical precision viticulture applications.
\end{abstract}
...
Grapevine health and fruit quality are critical determinants of vineyard productivity and wine quality, making early detection and accurate assessment of leaf conditions essential for precision viticulture. Traditionally, disease and physiological stress monitoring has relied on manual inspection, which is labor-intensive, subjective, and prone to error. Recent advances in computer vision and machine learning have transformed this domain, enabling automated analysis of grapevine leaves through high-resolution imagery. In particular, deep learning approaches, such as convolutional neural networks (CNNs), have demonstrated remarkable performance in capturing intricate morphological and spectral patterns associated with leaf health and ripening stages \cite{Zhang2024GMamba}.

Supportive studies highlight the efficacy of these approaches. CNN-based classification models have achieved accuracies exceeding 95\% \cite{Sagar2025}, while hybrid architectures incorporating multi-backbone fusion or transformer modules have reported near-perfect performance \cite{AlNasseri2025}. Object detection models, such as YOLO variants, have shown strong potential for in-field deployment, reaching mean average precision (mAP) up to 95\% \cite{Zhou2025YOLOGrape}. Complementary techniques including spectroscopic sensing and machine learning approaches like support vector machines and chemometric regression have further enabled nutrient assessment and early pathogen detection, with reported R² values up to 0.90 for macro- and micronutrient prediction \cite{Manzano2025}. Collectively, these results underscore the transformative potential of combining deep learning, attention mechanisms, and data-driven feature engineering in viticultural applications.Despite these advancements, several challenges remain. Most existing models are either computationally intensive or rely on large-scale datasets, limiting their applicability in small-scale vineyards or resource-constrained settings. Moreover, while classification accuracy is often high, many studies focus solely on binary disease detection or single-stage ripening classification, neglecting the nuanced multi-stage progression of grapevine leaves. Real-time deployability and lightweight architectures suitable for edge devices are rarely addressed, leaving a gap between research performance and practical vineyard implementation.

To address these gaps, this paper proposes \textit{GrapevineNet\_SE}, a lightweight transfer learning model optimized for multi-stage grapevine leaf classification on a relatively small dataset (~4,000 images). The model leverages a frozen EfficientNet-B0 backbone with selective feature extraction, enhanced by a lightweight squeeze-and-excitation attention module, and a compact classification head. By fine-tuning only a subset of the network parameters, \textit{GrapevineNet\_SE} achieves high accuracy while maintaining low computational cost, making it suitable for real-time applications on edge devices.

The key contributions of this work are as follows:
\begin{itemize}
    \item A lightweight, multi-stage grapevine leaf classification model that balances performance and computational efficiency.
    \item Integration of a squeeze-and-excitation attention mechanism to enhance feature representation and improve stage-wise classification accuracy.
    \item Demonstration of robust performance on training, validation, and test splits, achieving a validation accuracy of 90\% and a test accuracy of 86.5\%, indicating generalizability to unseen data.
\end{itemize}


The remainder of this paper is structured as follows. Section \ref{sec:lr} reviews relevant literature on grapevine leaf disease detection, including \ac{dl} architectures, attention mechanisms, and classical machine learning approaches. Section \ref{sec:methodology} details the materials and methods, covering dataset description, preprocessing pipelines, model architecture, and training strategy. Section \ref{sec:results} presents experimental results, performance evaluation, and comparative analysis with baseline pretrained models. Finally, Section \ref{sec:con} concludes the paper with key findings, discussion of limitations, and directions for future work.
...
\begin{table}[h!]
    \caption{Distribution of physical images across dataset splits for grapevine Pierce disease stages.}
    \centering
    \begin{tabular}{m{0.18\columnwidth} m{0.18\columnwidth} m{0.18\columnwidth} m{0.18\columnwidth}}
        \toprule
...
\STATE Freeze first 6 blocks of \texttt{features} (~75\% layers)
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{images/test_samples_visualization.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{images/confusion_matrix_test.png}}
