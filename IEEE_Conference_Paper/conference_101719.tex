...
The backbone of \textit{GrapevineNet\_SE} is based on EfficientNet-B0. EfficientNet-B0 provides hierarchical feature extraction from input images. The model uses pretrained weights to retain generic visual features while also adapts to the target dataset. Approximately 75\% of the early layers were frozen. It allows only the later layers to be fine-tuned, which balances transfer learning with task specific adaptation. Listing~\ref{lst:backbone} shows backbone structure.

\begin{lstlisting}[language=Python, caption=Backbone Initialization, label={lst:backbone}]
# Pretrained EfficientNet-B0 backbone
self.backbone = models.efficientnet_b0(weights="IMAGENET1K_V1")

# Freeze ~75% of early layers
...
Next part is a lightweight \ac{se} module that is applied to the final feature maps. It performs global average pooling. Than there is a small bottleneck with ReLU and sigmoid activations to the compute channel-wise attention. The attention weights scale the feature maps. It emphasizes the most informative channels to improve classification. Listing~\ref{lst:semodule} shows the \ac{se} module.

\begin{lstlisting}[language=Python, caption=\ac{se} Module, label={lst:semodule}]
# Lightweight SE attention module
...
The last part is classification head. It maps the attended feature maps to final predictions. It includes a fully connected layer, ReLU activation, dropout for regularization. At last, a final linear layer for the outputs corresponding to the target classes. Classification head is given in Listing~\ref{lst:classhead}.

\begin{lstlisting}[language=Python, caption=Classification Head, label={lst:classhead}]
# Classification head
...
During the forward pass, the input images first pass through the backbone to extract feature maps. Then \ac{se} module computes channel-wise attention and scales the features. Finally, the classification head processes those features and gives the class predictions. As shown in Listing~\ref{lst:fpass}.

\begin{lstlisting}[language=Python, caption=Forward pass for GrapevineNet\_SE, label={lst:fpass}]
def forward(self, x):
...
The model is initialized and moved to the available GPU device. We used google colab. A summary of the network is generated to visualize layer dimensions and parameters. Given in Listing~\ref{lst:msummary}.

\begin{lstlisting}[language=Python, caption=Model Init and Summary, label={lst:msummary}]
device = "cuda" if torch.cuda.is_available() else "cpu"
model = GrapevineNet_SE(num_classes=4, dropout=0.3).to(device)
