...
\begin{figure*}
\centerline{\includegraphics[width=0.7\textwidth]{images/test_samples_visualization.png}}
...
\begin{figure*}
\centerline{\includegraphics[width=\textwidth]{images/gradcam.png}}
\caption{\ac{gradcam} based explainability of \textit{SEPierceStageNet} on representative test images. For each sample, the original image is shown with the \ac{gradcam} heatmap overlay, highlighting the regions most influential for the model's prediction. Correct predictions are indicated with a checkmark, while incorrect predictions are marked with a cross.}
...
To interpret the decision-making process of our trained \ac{dl} model, we employed \ac{gradcam} \cite{Selvaraju_2017_ICCV}. \ac{gradcam} generates visual explanations by highlighting regions in the input images that contribute most to the predicted class. For our analysis, we selected one representative test image per predicted class and generated \ac{gradcam} overlays on the original images. The overlays reveal that the model consistently focuses on the leaf regions relevant to the grape development stage, confirming that the network relies on meaningful visual cues rather than background artifacts. Displaying the original and Grad-CAM heatmaps side by side provides a clear visual understanding of the spatial attention of the model, thereby improving interpretability and trustworthiness of the predictions. Given in Figure~\ref{fig:gradcam_explainability}
