...
\begin{figure*}[htbp]
	...
	\caption{{EfficientNet-B0} backbone for our \textit{SEPierceStageNet}.}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.7\columnwidth]{images/eb3.png}}
...
Although \textit{EfficientNet-B0} contains \ac{se} blocks in MBConv block, we add an additional trainable \ac{se} module at the feature head for better adaptation to our dataset. This module performs task-specific channel recalibration on the final convolutional features. Resulting in enhanced validation accuracy. Maintains a small training–validation gap. Adding this lightweight module eliminates need of unfreezing more backbone layers. More backbone layer could increase overfitting risk. \ac{se} provides feature refinement with minimal parameter overhead. As given in Section~\ref{sec:results}.
...
During the forward pass, the input images first pass through the backbone to extract feature maps. Then \ac{se} module computes channel-wise attention. Finally, the classification head processes those features and gives the class predictions. Shown in Listing~\ref{lst:fpass}.
...
In addition to this, confusion matrix was also visualized to analyze classification patterns. Per-class classification reports generated to provide detailed performance across all disease stages. These statistics are given in Section~\ref{sec:results}.
...
\begin{figure}[H]
	\centerline{\includegraphics[width=\columnwidth]{images/per_class_accuracy_all_sets.png}}
