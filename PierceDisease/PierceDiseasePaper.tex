...
\title{SEGrapeStageNet: SE-Attention EfficientNet for Grapevine Pierce’s Disease Stage Classification}
...
To address these gaps, this paper proposes \textit{SEGrapeStageNet}, a transfer learning model optimized for multi-stage grapevine leaf Pierce's Disease classification on a relatively small dataset. The model uses a frozen EfficientNet-B0 \cite{pmlr-v97-tan19a} backbone with selective feature extraction. It is enhanced by a lightweight \ac{se} attention \cite{Hu_2018_CVPR} module, and a compact classification head. By fine-tuning only a subset of the EfficientNet-B0 parameters, \textit{SEGrapeStageNet} achieves high accuracy of approximately 95\%. While  also maintained a low computational cost, making it suitable for real-time vineyard applications on edge devices.
...
The primary model, \textit{SEGrapeStageNet}, is based on EfficientNet-B0. It uses transfer learning. As shown in Algorithm~\ref{alg:segrapestagenet}, the architecture integrates a lightweight \ac{se} module to enhance feature discrimination. At the end is a custom classification head for final predictions. Figure~\ref{fig:model} illustrates the overall model structure. For benchmarking, standard EfficientNet-B0 and ResNet50 architectures were implemented under identical conditions.

\begin{figure*}[htbp]
\centerline{\includegraphics[width=0.9\textwidth]{images/model.png}}
	\caption{\textit{SEGrapeStageNet} Model architecture}
	\label{fig:model}
\end{figure*}

\begin{algorithm}
\caption{SEGrapeStageNet Training, Validation, and Testing}
\label{alg:segrapestagenet}
\begin{algorithmic}[1]
...
\subsubsection{\ac{se} Attention Module}
Next part is an \ac{se} attention module that is applied to the final feature maps. It performs global average pooling. Than there is a small bottleneck with ReLU and sigmoid activations to the compute channel-wise attention. The attention weights scale the feature maps. It emphasizes the most informative channels to improve classification. Listing~\ref{lst:semodule} shows \ac{se} module details.
...
\begin{lstlisting}[style=ieeelisting, frame=none,
caption={Forward Pass of SEGrapeStageNet},
label={lst:fpass}]
Input: Image batch X
...
\begin{figure*}[!t]
    ...
    \caption{\textit{SEGrapeStageNet} performance metrics across training and validation subsets}
    \label{fig:performance}
\end{figure*}

\section{Results}
\label{sec:results}
\textit{SEGrapeStageNet} demonstrated strong and consistent performance on training, validation, and test datasets. It outperforms baseline architectures EfficientNet-B0 and ResNet-50. The model was trained for up to 20 epochs. We used an ImageNet pretrained EfficientNet-B0 backbone combined with an \ac{se} module, custom classification head, optimized using AdamW. The pretrained backbone was fine-tuned with a learning rate of $1\times10^{-4}$. The classifier was trained at $1\times10^{-3}$.  Cross-entropy loss with label smoothing of 0.1 was applied. A one-cycle learning rate scheduler with a 40\% warm up period followed by cosine annealing was used. Mixed precision training accelerated computations and reduced the memory usage. We implemented Early stopping with a patience of 5 epochs to prevent overfitting. The best performing model based on validation accuracy was selected.

Training and validation metrics recorded for each epoch; including accuracy, precision, recall, and F1-score. The model showed steady improvements with an early training accuracy of 51.20\% and validation accuracy of 63.25\% after the first epoch. By epoch 10, training accuracy reached 84.86\% and validation accuracy 82.75\%, showing an effective learning. The final evaluation metrics are summarized in Table~\ref{tab:segrapestagenet_metrics}.

\begin{table}[H]
\caption{Training, Validation, and Test Metrics for \textit{SEGrapeStageNet}}
\centering
...
\bottomrule
\end{tabular}
\label{tab:segrapestagenet_metrics}
\end{table}
...
\caption{\textit{SEGrapeStageNet} model prediction samples for test data. Correctly predicted samples shown in green text, misclassifications in red text}
\label{fig:predictions}
\end{figure}

Class-wise evaluation indicates high accuracy across all disease stages. Misclassifications primarily occurred between adjacent disease stages Stage 2 and Stage 3. So errors are systematic and are linked to visual similarities in these two stages not random predictions. Figure~\ref{fig:perclass} illustrates the accuracy of each class on training, validation, and test sets, showing a stable performance in all classes.

\begin{figure}[H]
\centerline{\includegraphics[width=0.9\columnwidth]{images/per_class_accuracy_all_sets.png}}
\caption{Class wise accuracies across training, validation, and test datasets}
\label{fig:perclass}
\end{figure}
...
Overall, the model exhibits reliable, and generalizable performance for grapevine disease stage classification. Given a high accuracy, precision, recall, and F1-scores across all subsets.

\section{Comparison and Analysis}
\label{sec:comparison}

To assess the effectiveness of \textit{SEGrapeStageNet}, we made a comprehensive comparison with two widely used baseline models: EfficientNet-B0 and ResNet50. Both baselines were trained under identical conditions as \textit{SEGrapeStageNet}. We used ImageNet pretrained weights, the AdamW optimizer, and a similar training schedule for up to 20 epochs on the same training and validation subsets. Detailed comparison given in Table~\ref{tab:model_comparison}.

\begin{table*}[!t]
\caption{Performance Comparison of \textit{SEGrapeStageNet} and Baseline Models Across Training, Validation, and Test Sets; Training accuracy corresponds to the epoch yielding the best validation performance}
\centering
\begin{tabular}{
m{0.30\textwidth}
m{0.10\textwidth}
...
Per-class evaluation further emphasizes the robustness of \textit{SEGrapeStageNet}. Both EfficientNet-B0 and ResNet50 showed lower performance on intermediate disease stages, particularly Stage 1 and Stage 2. EfficientNet-B0 recorded an F1-score of 0.69 for Stage 1 and 0.55 for Stage 2, whereas ResNet50 underperformed with F1-scores of 0.57 and 0.51, respectively. Misclassifications primarily occurred between adjacent disease stages due to subtle visual similarities, a limitation mitigated in \textit{SEGrapeStageNet} through attention mechanisms and an optimized classifier.
...
\end{itemize}
The significant improvement of \textit{SEGrapeStageNet} over the baselines can be attributed to three key architectural enhancements: 
\begin{enumerate}
