...
\title{SEPierceStageNet: Explainable EfficientNet for Grapevine Pierce’s Disease Stage Classification}
...
To address these gaps, this paper proposes \textit{SEPierceStageNet}, a transfer learning model optimized for multi-stage grapevine leaf Pierce's Disease classification on a relatively small dataset. The model uses a frozen \textit{EfficientNet-B0} \cite{pmlr-v97-tan19a} backbone with selective feature extraction. It is enhanced by a lightweight \ac{se} attention \cite{Hu_2018_CVPR} module, and a compact classification head. By fine-tuning only a subset of the \textit{EfficientNet-B0} blocks, \textit{SEPierceStageNet} achieves accuracy of approximately 97\%. While  also maintained a low computational cost that makes it suitable for real-time vineyard applications on edge devices. We also add model explainability with \ac{gradcam}. The key contributions of this paper include:
\begin{itemize}
...
\begin{figure*}[htbp]
	...
	\caption{Proposed Architecture of \textit{SEPierceStageNet} with \textit{EfficientNet-B0} backbone and \ac{se} attention.}
	\label{fig:model}
\end{figure*}
...
\begin{figure*}[htbp]
	\centerline{\includegraphics[width=0.9\textwidth]{images/efficientnetb0.png}}
	\caption{{EfficientNet-B0} backbone for \textit{SEPierceStageNet}.}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\STATE Freeze first few feature blocks
...
The primary model, \textit{SEPierceStageNet}, is based on \textit{EfficientNet-B0}. It uses transfer learning. As shown in Algorithm~\ref{alg:SEPierceStageNet}, the architecture integrates a lightweight \ac{se} module to enhance feature discrimination for disease classification. At the end is a custom classification head for final predictions. Figure~\ref{fig:model} illustrates the overall model structure. For benchmarking, standard \textit{EfficientNet-B0} architecture was implemented under identical conditions as our proposed model.

\subsubsection{Backbone}
The backbone of the model is based on \textit{EfficientNet-B0}. It provides feature extraction from input images. The model uses pretrained ImageNet weights to retain generic visual features while also adapting to the target dataset. Early blocks were frozen in 3 different experiments. First experiment with first 3 blocks frozen, second with 4 frozen blocks and third with 5 blocks frozen. Backbone shown in Figure~\ref{fig:efficientnetb0} and Listing~\ref{lst:backbone}. Only the later layers were fine-tuned Figure~\ref{fig:efficientnetb03}, Figure~\ref{fig:efficientnetb04} and Figure~\ref{fig:efficientnetb05}.
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.9\columnwidth]{images/eb3.png}}
	\caption{{EfficientNet-B0} backbone with first 3 blocks frozen.}
	\label{fig:efficientnetb03}
\end{figure}
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.9\columnwidth]{images/eb4.png}}
	\caption{{EfficientNet-B0} backbone with first 4 blocks frozen.}
	\label{fig:efficientnetb04}
\end{figure}
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.9\columnwidth]{images/eb5.png}}
	\caption{{EfficientNet-B0} backbone with first 5 blocks frozen.}
	\label{fig:efficientnetb05}
\end{figure}
...
\begin{table}[h!]
    \caption{SE attention configuration.}
    \centering
    \begin{tabular}{lc}
        \toprule
...
\label{sec:results}
\textit{SEPierceStageNet} provides strong and consistent results on training, validation, and test datasets. It outperforms baseline architectures \textit{EfficientNet-B0}, \textit{MobileNetV3Small} and \textit{ResNet-50} on our dataset.
...
\subsection{\textit{SEPierceStageNet} With First Three Backbone Blocks Frozen}
The proposed SEPierceStageNet trained for 20 epochs with three blocks frozen. The model continuously improved in both training and validation performance. The epoch with the highest validation accuracy (Epoch 18) is used to report the final results. At this epoch the model gained training accuracy, precision, recall, and F1 score of 0.9691. On the validation set it achieved 0.9475 accuracy, 0.9485 precision, 0.9475 recall and F1 score of 0.9476. Evaluation on the separate test set gave comparable results. Test accuracy of 0.9451, precision of 0.9451, recall of 0.9451, and F1 score .The model had strong generalization performance. The final evaluation metrics are summarized in Table~\ref{tab:SEPierceStageNet3_metrics} and Figure~\ref{fig:performance3}.

\begin{table}[H]
	\caption{Best Validation Epoch results: Training, validation, and test metrics of \textit{SEPierceStageNet} with 3 blocks frozen.}
	\centering
	\begin{tabular}{m{0.3\columnwidth} m{0.1\columnwidth} m{0.1\columnwidth} m{0.1\columnwidth} m{0.1\columnwidth}}
		...
		\bottomrule
	\end{tabular}
	\label{tab:SEPierceStageNet3_metrics}
\end{table}
...
\begin{figure}[h!]
	...
	\caption{Training and validation curves for \textit{SEPierceStageNet} 3 backbone blocks frozen.}
...
\begin{figure}
	...
	\caption{Confusion matrix for \textit{SEPierceStageNet} With first three backbone blocks frozen on test dataset with true and predicted class labels.}
	\label{fig:cmatrix3}
\end{figure}
...
\subsection{\textit{SEPierceStageNet} With First Four Backbone Blocks Frozen}
For the model configuration with four layers frozen trained for 20 epochs. Precision, recall, and F1-score followed a similar trend, with the model achieving a validation F1-score of 0.9297 at the conclusion of training. These results suggest that freezing four layers allowed the model to effectively use pre-trained representations while also adapt to the target task. Results metrics of best epoch (17 epoch) were slightly lower than previous with 3 blocks of backbone frozen. The final evaluation metrics are summarized in Table~\ref{tab:SEPierceStageNet4_metrics} and Figure~\ref{fig:performance4}.

\begin{table}[H]
	\caption{Best Validation Epoch results: Training, validation, and test metrics of \textit{SEPierceStageNet} with 4 blocks frozen.}
	\centering
	\begin{tabular}{m{0.3\columnwidth} m{0.1\columnwidth} m{0.1\columnwidth} m{0.1\columnwidth} m{0.1\columnwidth}}
		\toprule
		\textbf{Dataset} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1 Score} \\
		...
		\bottomrule
	\end{tabular}
	\label{tab:SEPierceStageNet4_metrics}
\end{table}
...
\begin{figure}[h!]
	...
	\caption{Training and validation curves for \textit{SEPierceStageNet} 4 backbone blocks frozen.}
...
\begin{figure}
	...
	\caption{Confusion matrix for \textit{SEPierceStageNet} With first four backbone blocks frozen on test dataset with true and predicted class labels.}
	\label{fig:cmatrix4}
\end{figure}

% \subsection{\textit{SEPierceStageNet} With First Five Backbone Blocks Frozen}
% The proposed SEPierceStageNet trained for 20 epochs with three blocks frozen. The model continuously improved in both training and validation performance. The epoch with the highest validation accuracy (Epoch 18) is used to report the final results. At this epoch the model gained training accuracy, precision, recall, and F1 score of 0.9691. On the validation set it achieved 0.9475 accuracy, 0.9485 precision, 0.9475 recall and F1 score of 0.9476. Evaluation on the separate test set gave comparable results. Test accuracy of 0.9451, precision of 0.9451, recall of 0.9451, and F1 score .The model had strong generalization performance. The final evaluation metrics are summarized in Table~\ref{tab:SEPierceStageNet_metrics}.
